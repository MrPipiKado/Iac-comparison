...
def test_sg_count():
    ...
    for env in configs.config_names:
        ...
        stack = Stack(app, "TestStack")
        network_layer_stack = NetworkLayerStack(stack, "NetworkLayerStack", env)
        sg_layer_stack = SGLayerStack(
            stack,
            "SGLayerStack",
            network_layer_stack.application_vpc,
            network_layer_stack.database_vpc,
            env,
            configs.configs_with_names[env],
        )
...
def test_alb_creation():
    ...
    for env in configs.config_names:
        ...
        application_layer = ApplicationLayerStack(
            stack,
            "ApplicationStack",
            network_layer_stack.application_vpc,
            network_layer_stack.app_public_subnets,
            sg_layer_stack.load_balancer_sg,
            sg_layer_stack.application_sg,
            env,
            configs.configs_with_names[env]["App"],
        )
...
def test_bastion_creation():
    ...
    for env in configs.config_names:
        ...
        bastion_layer = BastionLayerStack(
            stack,
            "BastionStack",
            network_layer_stack.application_vpc,
            network_layer_stack.app_public_subnets,
            sg_layer_stack.bastion_sg,
            configs.configs_with_names[env]["BastionHost"],
        )
