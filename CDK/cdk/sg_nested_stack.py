...
class SGLayerStack(NestedStack):
    ...
    def __init__(self, scope: Construct, construct_id: str, app_vpc: ec2.IVpc, db_vpc: ec2.IVpc, general_config: dict, **kwargs) -> None:
        ...
        self.database_sg.add_ingress_rule(peer=self.application_sg, connection=ec2.Port.tcp(3306))
        ...
        if general_config["BastionHost"]["enabled"]:
            ...
            if general_config["BastionHost"]["access"] == "my_ip":
                self.bastion_host_sg.add_ingress_rule(ec2.Peer.ipv4(MyIP().get_ip_cidr()), ec2.Port.tcp(22), "SSH access")
            elif general_config["BastionHost"]["access"] == "any":
                self.bastion_host_sg.add_ingress_rule(ec2.Peer.any_ipv4(), ec2.Port.tcp(22), "SSH access")
            else:
                self.bastion_host_sg.add_ingress_rule(ec2.Peer.ipv4(general_config["BastionHost"]["access"]), ec2.Port.tcp(22), "SSH access")

            self.application_sg.add_ingress_rule(peer=self.bastion_host_sg, connection=ec2.Port.tcp(22))
